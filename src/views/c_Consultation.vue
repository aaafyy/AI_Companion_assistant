          ...
          <div class="warm-suggestion" v-if="currentEmotion.suggestion">
            ...
            <div class="suggestion-content">
              ...
              <div class="suggestion-text">
                {{ currentEmotion.suggestion || "暂无建议" }}
              </div>
            </div>
          </div>
        </div>
              ...
              <div class="session-title">
                ...
                <div class="session-stats">
                  <span
                    ...
                    {{ session.messageeCount || 0 }}</span
                  >
                  <span></span>
                  <span
                    ><el-icon><Clock /></el-icon>
                    {{ session.lastMessageTime || 0 }}</span
                  >
                  <span></span>
                </div>
              </div>
        ...
        <div class="message-item ai-message" v-if="messages.length === 0">
          ...
          <div class="message-content">
            <div class="message-bubble">
              <p>
                您好，我是小暖，您的心理健康AI助手，很高兴陪伴您，为您提供温暖的心理支持，请告诉我，今天您感觉怎么样
              </p>
            </div>
...
const chatAvatarUrl = new URL("@/assets/images/like.png", import.meta.url).href;
const usersUrl = new URL("@/assets/images/users.png", import.meta.url).href;
// 定义一个当前会话对象
...
const startNewSession = (message) => {
  ...
  startSession(sessionParams).then((res) => {
    ...
    if (currentSession.value && currentSession.value.status === "TEMP") {
      // 保持 currentSession.value 的对象引用不变，只更新 sessionData 中包含的属性，其他属性保持不变
...
const startAiResponse = (sessionId, userMessage) => {
  ...
  fetchEventSource("/api/psychological-chat/stream", {
    ...
    // sinal,处理请求信号
    signal: ctrl.signal,
    // onopen 事件处理函数,检查响应头是否为流式数据
    onopen: (response) => {
      console.log(response);
      if (response.headers.get("Content-Type") !== "text/event-stream") {
        ElMessage({
          message: "浏览器返回的不是流式数据，无法正常显示",
          type: "error",
        });
      }
    },
    // onmessage 事件处理函数,处理流式数据
    onmessage: (event) => {
      console.log(event);
...
const getSessionpage = () => {
  getSessionList({ pagNum: 1, pageSize: 10 }).then((res) => {
    console.log(res);
